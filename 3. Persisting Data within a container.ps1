...
docker exec -it testcontainer4 bash
...
docker run -d -p 15999:1433 `
    -v sqlserver:/sqlserver `
        --env ACCEPT_EULA=Y --env SA_PASSWORD=Testing11@@ `
            --name testcontainer5 microsoft/mssql-server-linux:latest
...
# blow away container
docker kill testcontainer5
docker rm testcontainer5
...
docker run -d -p 16100:1433 `
    -v sqlserver:/sqlserver `
        --env ACCEPT_EULA=Y --env SA_PASSWORD=Testing11@@ `
            --name testcontainer6 microsoft/mssql-server-linux:latest
...
# clean up
docker kill testcontainer6
docker rm testcontainer6
